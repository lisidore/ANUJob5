  ...
  if (nrow_ev!=0 && sum(dendistrt)>0) { //start of top_level_if_1
    ...
    for(int i = 0; i < nrow_ev; i++){ //start of for loop
      ...
      if(events_type[i] == 1){ // start of outer_if_1
        ...
        Rcpp::IntegerVector temp_vec7;
        Rcpp::NumericVector temp_vec8;
        Rcpp::NumericVector temp_vec9;
        Rcpp::NumericVector temp_vec10;
        Rcpp::NumericVector temp_vec11;
        double anew;
        // double h = 5.1; only for testing until h is passed as an argument 
        ...
        Rcpp::NumericVector double_temp_vec7 = Rcpp::NumericVector (temp_vec7); //typecast from IntegerVec to NumericVec    **********
        Rcpp::NumericVector double_tmpt = Rcpp::NumericVector (tmpt); //typecast from IntegerVec to NumericVec    **********
        temp_vec8 = dendistrt * double_temp_vec7;
       temp_vec9 = double_tmpt * double_temp_vec7;
        
        anew = sum(temp_vec8) - sum(temp_vec9);
        temp_vec10 = dendistrt * double_temp_vec7 * double_temp_vec7;
        temp_vec11 = double_tmpt * double_temp_vec7 * double_temp_vec7;
       ...
       b = sum(temp_vec10) - sum(temp_vec11) - (anew * anew);
